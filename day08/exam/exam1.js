...
let person = {name: '허유현', age: 23  , city:'광명시' }
console.log(person)
...
const product = { name: '노트북', price: 1200000, stock: 50 };
let 자산가치 = product.price * product.stock
console.log(자산가치)

/* 예제 5: 대괄호 표기법 사용하기
다음 user 객체가 있습니다. 대괄호 표기법을 사용하여 
job-title 속성의 값('개발자')을 콘솔에 출력하시오.
const user = { name: '김코딩','job-title': '개발자'}; */
// console.log(user.job-title) // 주의할점 : 속성명에 특수문자가 있거나 변수로 정의한 경우 .점이 아닌 [] 사용.
const user = { name: '김코딩','job-title': '개발자'};
...
let 주소 = student.address; console.log( 주소.city )
...
for( let index = 0 ; index <= 속성명들.length-1 ; index++){
    let 속성명 = 속성명들[ index ];  // index 번째의 속성명 호출
    if( inventory[ 속성명 ]< 10 ){  // index 번째의 속성값이 10 미만이면 
        console.log( 속성명 )
    }
}
/* 예제 10: 객체 배열 순회하기
...
for( let index = 0 ; index <= students.length - 1 ; index++ ){
    let 학생객체 = students[index] ;  // index번째의 학생객체 호출 
    console.log( `이름: ${학생객체.name} 전공: ${ 학생객체.major}` )
}
